...
class MLP:
    ...
    def train(self, inputs, targets, eta, niterations):
        ...
        inputs is a numpy array of shape (num_train, D) containing the training images
                    consisting of num_train samples(no. of rows) each of dimension D (no. of columns).
        ...
        """
        ndata = np.shape(inputs)[0]  # number of data samples(rows)
        # adding the bias- an array of -1s to the end of the inputs.
        ...
        for n in range(niterations):
            ...
            updatew1 = eta*(np.dot(np.transpose(self.inputs),deltah1[:,:-1])) + self.momentum*updatew1
    ...
    def forwardPass(self, inputs):
        ...
        self.hidden1 = 1.0/(1.0+np.exp(-beta*self.hidden1))
        ...
        self.hidden2 = 1.0/(1.0+np.exp(-beta*self.hidden2))
        ...
        expOut = np.exp(outputs)
        outputs = expOut/expOut.sum()
